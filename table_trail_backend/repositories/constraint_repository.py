...
from table_trail_backend.schemas.constraint_schema import (
    CreateConstraint,
    UpdateConstraint,
)
...
class ConstraintsRepository:
    ...
    async def create_column_constraint(self, column_id: int, constraint_id: int):
        new_column_constraint = ConstraintColumn(column_id=column_id, constraint_id=constraint_id)
    ...
    async def get_constraint_by_id(self, table_id: int, constraint_id: int):
        constraint = await self.db.execute(
            select(Constraints).where(and_(Constraints.table_id == table_id, Constraints.id == constraint_id))
        )
        return constraint.scalar_one_or_none()

    async def get_table_constraints(self, table_id: int):
        constraints = await self.db.execute(select(Constraints).where(Constraints.table_id == table_id))
        return constraints.scalars().all()

    async def get_column_constraints(self, column_id: int):
        constraints = await self.db.execute(select(ConstraintColumn).where(ConstraintColumn.column_id == column_id))
        return constraints.scalars().all()
