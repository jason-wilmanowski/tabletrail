...
def test_generate_markdown_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "table_trail_backend.export.generators.markdown_generator.Environment", lambda **kwargs: fake_environment
    )
    ...
    fake_environment.get_template.assert_called_once_with("database_markdown.md.j2")
